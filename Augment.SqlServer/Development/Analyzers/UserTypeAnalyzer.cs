...
namespace Augment.SqlServer.Development.Analyzers
{
    public class UserTypeAnalyzer
    {
        ...
        public void Analyze(SqlObject source, SqlObject target)
        {
            ...
            string sourceDefinition = _analyzer.Connection
                .Query<string>(sql.FormatArgs(tempObj.ObjectName))
                .First();

            string targetDefinition = _analyzer.Connection
                .Query<string>(sql.FormatArgs(target.ObjectName))
                .First();
